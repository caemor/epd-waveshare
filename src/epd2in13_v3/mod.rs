...
macro_rules! frame_assert {
    ($buffer:expr) => {
        assert_eq!(
            $buffer.len(),
            crate::buffer_len(
                crate::epd2in13_v3::WIDTH as usize,
                crate::epd2in13_v3::HEIGHT as usize
            )
        );
    };
}

/// Full size buffer for use with 2in13 v3 EPD
    ...
    DELAY: DelayUs<u32>,
{
    ...
    fn update_and_display_frame(
        ...
        if self.refresh == RefreshLut::Quick {
            self.set_partial_base_buffer(spi, buffer, delay)?;
        }
        Ok(())
    }
...
mod tests {
    ...
    // test failure of 2in13 v3 update_frame(...) assert
    #[test]
    fn buffer_size_assert() {
